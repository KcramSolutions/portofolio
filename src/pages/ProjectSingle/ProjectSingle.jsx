...
import { Badge, Button, Carousel, Col, Container, Image, Ratio, Row, Stack } from 'react-bootstrap';
			...
			<Container style={{ width: "50%", margin: "5rem auto", minWidth: "300pt" }}>
				...
				<Row>
					...
					{project.data.galery != null ? (<Col>
						...
						<Carousel className={fullscreen ? "carousel-fullscreen" : ""} onClick={() => setFullscreen(true)}>
							{project.data.galery.map(img => {
								return (
									<Carousel.Item key={img.id}>
										<Image src={img.source_url} fluid/>
									</Carousel.Item>
								)
							})}
						</Carousel>
						{fullscreen && (
							<Button className="close-button" onClick={()=>setFullscreen(false)}>
								X
							</Button>
						)}
					</Col>) : (<></>)}
